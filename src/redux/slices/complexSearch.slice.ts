import {IRecipeInitialState, IRecipesServices, RecipesInterface,IParams} from "../../interfaces";
...
export const getRecipes = createAsyncThunk<IRecipesServices,IParams, {rejectWithValue:boolean}>(
    ...
    async ({query,cuisine},{rejectWithValue})=>{
        try {
            const {data} = await searchRecipesService.getSearchRecipes(query,cuisine);
            return data
        }catch (e) {
